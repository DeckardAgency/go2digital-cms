...
@Component({
  selector: 'app-lab-category-form',
  standalone: true,
  imports: [
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <p-button icon="pi pi-arrow-left" severity="secondary" [text]="true" [rounded]="true" (onClick)="router.navigate(['/lab/categories'])" />
          <div>
            <h1 class="text-2xl font-semibold text-surface-900 dark:text-surface-0">{{ isEditMode() ? 'Edit Category' : 'New Category' }}</h1>
            <p class="text-surface-500 dark:text-surface-400 text-sm mt-0.5">{{ isEditMode() ? 'Update lab category' : 'Create a new lab category' }}</p>
          </div>
        </div>
...
export class LabCategoryFormComponent implements OnInit {
  ...
  private loadCategory(id: string): void {
    this.labService.getCategory(id).subscribe({
      next: (cat) => {
      ...
      error: () => { this.messageService.add({ severity: 'error', summary: 'Failed to load category' }); this.router.navigate(['/lab/categories']); },
    });
  }
  ...
  onSave(): void {
    ...
    const request$ = this.isEditMode() ? this.labService.updateCategory(this.categoryId()!, payload) : this.labService.createCategory(payload);
    request$.subscribe({
      next: () => { this.messageService.add({ severity: 'success', summary: this.isEditMode() ? 'Category updated' : 'Category created' }); this.router.navigate(['/lab/categories']); },
