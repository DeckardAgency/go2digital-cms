      ...
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
          <p-button
            ...
            severity="secondary"
            [text]="true"
            (onClick)="router.navigate(['/lab/categories'])" />
...
export class LabCategoryFormComponent implements OnInit {
  ...
  private loadCategory(id: string): void {
    this.labService.getCategory(id).subscribe({
      ...
      error: () => {
        ...
        this.router.navigate(['/lab/categories']);
      },
    });
  }
  ...
  onSave(): void {
    ...
    const request$ = this.isEditMode()
      ? this.labService.updateCategory(this.categoryId()!, payload)
      : this.labService.createCategory(payload);
