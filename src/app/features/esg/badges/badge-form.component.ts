    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <p-button icon="pi pi-arrow-left" severity="secondary" [text]="true" [rounded]="true" (onClick)="router.navigate(['/esg/badges'])" />
          <div>
            <h1 class="text-2xl font-semibold text-surface-900 dark:text-surface-0">
              {{ isEditMode() ? 'Edit Badge' : 'New Badge' }}
            </h1>
            <p class="text-surface-500 dark:text-surface-400 text-sm mt-0.5">
              {{ isEditMode() ? 'Update vision badge details' : 'Create a new vision badge' }}
            </p>
          </div>
        </div>
...
export class BadgeFormComponent implements OnInit {
  ...
  onSave(): void {
    const payload: any = { sortOrder: this.sortOrder, translations: this.translations() };
    ...
    request$.subscribe({
      next: () => {
        this.messageService.add({ severity: 'success', summary: this.isEditMode() ? 'Badge updated' : 'Badge created' });
        this.router.navigate(['/esg/badges']);
      },
      error: () => this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to save badge' }),
    });
  }
}
