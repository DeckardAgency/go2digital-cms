    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <p-button icon="pi pi-arrow-left" severity="secondary" [text]="true" [rounded]="true" (onClick)="router.navigate(['/homepage/tracking'])" />
          <div>
            <h1 class="text-2xl font-semibold text-surface-900 dark:text-surface-0">{{ isEditMode() ? 'Edit Tracking Feature' : 'New Tracking Feature' }}</h1>
            <p class="text-surface-500 dark:text-surface-400 text-sm mt-0.5">Tracking & analytics section feature</p>
          </div>
        </div>
          ...
          <div class="bg-surface-0 dark:bg-surface-900 rounded-xl border border-surface-200 dark:border-surface-700 p-6">
            ...
            <div class="flex flex-col gap-3 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium text-surface-900 dark:text-surface-0">#10 Tracking & Analytics</span>
              </div>
              <div class="flex justify-between">
...
export class TrackingFormComponent implements OnInit {
  ...
  private loadItem(id: string): void {
    this.homepageService.getTrackingFeature(id).subscribe({
      next: (item) => {
        this.icon = item.icon || '';
      ...
      error: () => {
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load tracking feature' });
        this.router.navigate(['/homepage/tracking']);
      },
    });
  }
  ...
  onSave(): void {
    ...
    const request$ = this.isEditMode()
      ? this.homepageService.updateTrackingFeature(this.itemId()!, payload)
      : this.homepageService.createTrackingFeature(payload);
    ...
    request$.subscribe({
      next: () => {
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: this.isEditMode() ? 'Tracking feature updated successfully' : 'Tracking feature created successfully',
        });
        this.router.navigate(['/homepage/tracking']);
      },
      error: () => {
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to save tracking feature' });
      },
    });
  }
}
